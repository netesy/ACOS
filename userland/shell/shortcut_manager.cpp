...
void DesktopShortcutManager::sync() {
    ...
    m_shortcuts.clear();
    if (m_grid) m_grid->clear_children();
...
void DesktopShortcutManager::sync_file(const char* dir, const char* name) {
    ...
    while (*line) {
        ...
        if (strcmp(line, "Name=") == 0 || (strlen(line) > 5 && memcmp(line, "Name=", 5) == 0)) {
            memcpy(app_name, line + 5, strlen(line + 5) + 1);
        } else if (strcmp(line, "Exec=") == 0 || (strlen(line) > 5 && memcmp(line, "Exec=", 5) == 0)) {
            memcpy(exec_path, line + 5, strlen(line + 5) + 1);
        } else if (strcmp(line, "Icon=") == 0 || (strlen(line) > 5 && memcmp(line, "Icon=", 5) == 0)) {
